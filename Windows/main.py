...
from core.engine import FayzeeEngine
...
def fayzee_loop(pause_event, registry, args):
    """
    Main loop for FAYZEE, running in a separate thread.
    Checks pause_event to determine if it should listen/process.
    """
    # Initialize Engine
    fayzee = FayzeeEngine(registry)

    if args.text:
        print("FAYZEE: Fayzee Online. Ready for command (Text Mode).")
    else:
        speak("Fayzee Online. Ready for command.")
    ...
    while True:
        ...
        if "quit" in user_query: 
            print("Shutting down FAYZEE loop...")
        ...
        if "fayzee" not in user_query and not is_direct:
            print(f"Ignored: {user_query}")
            continue
            
        clean_query = user_query.replace("fayzee", "").strip()
        
        try:
            print(f"Thinking: {clean_query}")
            response = fayzee.run_conversation(clean_query)
            ...
            if response:
                if args.text:
                    print(f"FAYZEE: {response}")
                else:
                    speak(response)
        except Exception as e:
            print(f"Main Loop Error: {e}")
            if args.text:
                print("FAYZEE: System error.")
            else:
                speak("System error.")

def main():
    parser = argparse.ArgumentParser(description="FAYZEE AI Assistant")
    ...
    # 3. Start FAYZEE Loop in Background Thread
    # Daemon thread so it dies when GUI closes
    t = threading.Thread(target=fayzee_loop, args=(pause_event, registry, args), daemon=True)
    t.start()
